 ...
 * @date   2013-6-19
 */
public class BlogTableListAdpater extends AbsBlogListAdapter {
	
	XMultiColumnListView blogTabletLV;
	
	public BlogTableListAdpater(Activity act, XMultiColumnListView LView){
		super(act);
		blogTabletLV = LView;
		blogTabletLV.setAdapter(this);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		aq.id(R.id.blogFace).image(blogList.get(position).GetAvator());
		aq.id(R.id.blogAuthor).text(blogList.get(position).GetAuthor());
		//TODO 还有一个时间变量
		aq.id(R.id.blogTime).text(blogList.get(position).GetUpdateTime());
		return convertView;
	}
